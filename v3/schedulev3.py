...
class DatabaseManager:
        ...
        try:
            ...
            with self._connect() as conn:
                ...
                for channel in channel_data:
                    ...
                    query = f"SELECT COUNT(*) FROM SCHEDULE WHERE end > '{now}' AND Channel = {channel_number}"
    ...
    def insert_into_schedule(self, channel_number, showtime, end, filepath, chapter, runtime):
        ...
        query = """INSERT INTO SCHEDULE (Channel, Showtime, End, Filepath, Chapter, Runtime) VALUES (?, ?, ?, ?, ?, ?)"""
        params = (
            channel_number,
...
class Ch2Scheduler(Scheduler):
    ...
    def get_next_playtime(self, media_TD):
        """ """
        if media_TD < timedelta(0):
            raise ValueError("Movie duration cannot be negative")

        media_end = (self.marker + media_TD).replace(second=0, microsecond=0)
        minutes_past_hour = media_end.minute

        if minutes_past_hour < 30:
            next_play_time = media_end.replace(minute=30)
        elif minutes_past_hour >= 30:
            next_play_time = media_end.replace(minute=0)
            next_play_time += timedelta(hours=1)
    ...
    def schedule(self):
        try:
            ...
            while self.marker < self.end_datetime:
                ...
                catalog.append({"Media": media["Filepath"], "Start": f"{self.marker.strftime('%H:%M:%S')}"})
            ...
            # for cat in catalog:
            #     self.log.info(f"{cat['Media']}-{cat['Start']}")
